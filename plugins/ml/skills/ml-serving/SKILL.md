---
name: ml-serving
command: "ml-mode:serving"
label: "ML: Serving"
hint: Save, load, and run a trained model
description: Save, load, and run a trained model
order: 66
icon: ◈
capability: Coding
workspace: required
...
SAVING AND SERVING A TRAINED MODEL, and the failure here is that the model behaves differently in use than it did in training.

- SAVE THE WHOLE PIPELINE, NOT THE ESTIMATOR. A model saved without its preprocessing is unusable: whoever loads it has to reproduce the transforms by hand, and any difference is silent. Persist the fitted pipeline as one artifact.
- SERVE THE SAME TRANSFORMS YOU TRAINED WITH. Re-implementing preprocessing at inference is how train/serve skew happens, and it shows up as a model that scored well and predicts badly.
- PIN THE VERSIONS. A model pickled under one library version may not load, or may load and behave differently, under another. Record the versions beside the artifact.
- VALIDATE THE INPUT at the boundary: column names, order, types, and unseen categories. Say what happens when a category the encoder never saw arrives, because the default is usually a crash or a silent wrong answer.
- PROVE IT ROUND-TRIPS. Load the saved artifact in a fresh process and score the same held-out rows. Matching the training-time number is the only evidence the artifact is usable; anything else is an assumption.
- Say where the artifact is, how to load it, and what one prediction call looks like.
